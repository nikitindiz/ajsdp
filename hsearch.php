...
$doc->addStyleSheet(JURI::Base(). 'templates/rpl_twenty_eight/html/mod_rpl_search/css/rpl_search_restyle.css');
...
jQuery(document).ready(function(e) {
  ...
  jQuery('#rpl_search_form<?php echo $mod_id;?> #rpl_search_li9 select').prepend('<option selected="selected"><?php echo JTEXT::_('BATH'); ?></option>');
  jQuery('#rpl_search_form<?php echo $mod_id;?> #rpl_search_li8 select').prepend('<option selected="selected"><?php echo JTEXT::_('BED'); ?></option>');
...
<div class="hsearch_form_wrapper">
  <div class="hsearch container rpl_search_form">
    ...
    <div id="rpl_search_<?php echo $mod_id;?>" class="rpl_search_mod<?php echo $moduleclass_sfx?> search_content_box">
      ...
      foreach  ($resulted_fields as $field)
      {
        ...
        $field_name_class = str_replace(' ', '_', strtolower(JTEXT::_($field['field']->name)));

        $currently_wrapper_top = '<span class="field_wrapper '.$field_name_class.'">';
        ...
        $currently_wrapper_bottom = '</span>';
        $currently .=  "\r\n";



        if(in_array($field['field']->id,$top_block_id)){
          $top_block .= $currently_wrapper_top;
          $top_block.=$currently;
          $top_block .= $currently_wrapper_bottom;
        }else{
      ...
      $top_block.='<span class="search_button_wrapper"><li class="search_btn_li"><button class="rpl_search_button button" onclick="rpl_do_search'.$mod_id.'()">'.JTEXT::_('SEARCH').'</button>';
      $top_block.= ($reset_button)?'<span class="reset button" onclick="rpl_do_reset'.$mod_id.'();">'.JTEXT::_('RESET').'</span>':'';
      $top_block.= '</li></span>';
      echo '<div class="top_block"><div class="fields_wrapper"><ul class="fields_list">'.$top_block.'</ul></div></div><div class="bott_block_wrapper"><div class="bott_block" style="display:none;"><ul class="fields_list">'.$bott_block.'</ul></div></div>';
      ?>
    </div>
